...
class CodesysNvl extends utils.Adapter {
    ...
    async onMainTimerInterval() {
        try {
            for (const gvlItem of this.gvlInfo) {
                // iterate through all active NVLs
                if (gvlItem.type === 'send') {
                    // NVLs to send
     ...
     */
    onServerError(err) {
        this.log.error(`Codesys-NVL server got Error: ${err} closing socket`);
    ...
    async onServerMessage(msg, info) {
        try {
            ...
            this.log.debug(
                `-> ${msg.length} bytes from ${info.address}:${info.port}: <${this.logHexData(msg_hex)}> org: <${msg.toString()}>`,
            );
            ...
            this.log.debug(
                `listId: ${telegram.listId}, subId: ${telegram.subId}, varSize: ${telegram.varSize}, teleSize: ${telegram.teleSize}, teleCounter: ${telegram.teleCounter}`,
            );
            ...
            if (telegram.subId >= gvlItem.gvlStructure.byteLength) {
                throw {
                    message: `Telegram has a subIndex higher than expected: ${telegram.subId}`,
                };
            } else if (telegram.teleSize !== msg.length) {
                throw {
                    message: `Telegram size (${msg.length}B) is different than the value in the header (${telegram.teleSize} Byte).`,
                };
            ...
            await this.setStateChangedAsync(
                `${this.namespace}.nvl.${telegram.listId}.info.lastReceived`,
                Date.now(),
                true,
            );
     ...
     */
    onClientError(err) {
        this.log.error(`Codesys-NVL broadcast sender got Error: ${err} closing socket`);
        // Reset the connection indicator
        this.setState('info.connection', false, true);
    ...
    onClientListening() {
        ...
        this.log.info(
            `Codesys-NVL broadcast sender ready on <${addr.address}> port <${addr.port}> proto <${addr.family}>`,
        );
    ...
    async sendTelegram(listId) {
        try {
            ...
            if (gvlItem.type !== 'send') {
                return;
            } // disabled or receive list

            if (gvlItem.aktMinGap > 0) {
                // wait for the min gap
            ...
            for (const stateToSend of Object.entries(statesToSend)) {
                ...
                switch (gvlStructure.children[variableToSend].type) {
                    ...
                    case 'STRING':
                        data[variableToSend] = String(valueToSend).substring(
                            0,
                            gvlStructure.children[variableToSend].byteLength - 1,
                        ); // shorten by 1 for trailing 0x0
                        break;

                    case 'WSTRING':
                        data[variableToSend] = String(valueToSend).substring(
                            0,
                            gvlStructure.children[variableToSend].byteLength / 2 - 1,
                        ); // shorten by 1 for trailing 0x0 and half for 16 bit characters
                        break;

                    case 'INT':
                        if (Number(valueToSend) > 32767) {
                            valueToSend = 32767;
                        }
                        if (Number(valueToSend) < -32768) {
                            valueToSend = -32768;
                        }
                    ...
                    default:
                        this.log.error(
                            `dataType: ${gvlStructure.children[variableToSend].type} not implemented for sending`,
                        );
                }
            }
    ...
    async loadGvlFiles() {
        try {
            ...
            for (const fileEntry of dirContent) {
                ...
                if (fileEntry.file.split('.').pop() !== 'gvl') {
                    continue;
                } // skip not gvl files
                ...
                if (validationResult === true) {
                    ...
                    if (
                        gvlFileXML.GVL.NetvarSettings.Pack !== undefined &&
                        gvlFileXML.GVL.NetvarSettings.Pack !== 'True'
                    ) {
                        this.log.warn(
                            `only "packed" nvl is supported. File ${fileName} is configured as unpacked or is invalid. Skip this gvl.`,
                        );
                        continue;
                    }
                    ...
                    if (typeof configEntry === 'object') {
                        ...
                        if (gvlFileContent === String(configEntry.fileContent)) {
                            // skip loading if file content has not changed
                    ...
                    const localListId =
                        gvlFileXML.GVL.NetvarSettings.ListIdentifier !== undefined
                            ? Number(gvlFileXML.GVL.NetvarSettings.ListIdentifier)
                            : 0;
                    if (localListId === 0) {
                    ...
                    if (this.idsToCreate[localListId] !== undefined) {
                        this.log.error(
                            `Error in GVL-Files. Double ListId ${localListId} in file: ${fileName} and ${this.idsToCreate[localListId].fileName}`,
                        );
                        this.configurationOk = false;
                        return;
                    }
            ...
            if (configDirty) {
                ...
                // @ts-expect-error works because gvlInfo will be set at any case
                instanceObject.native.gvlInfo = this.gvlInfo;
                // @ts-expect-error works see above
                await this.setForeignObject(`system.adapter.${this.namespace}`, instanceObject);
            }
        ...
        try {
            ...
            for (const key in this.idsToCreate) {
                ...
                for (const element of this.objectsTemplates.nvls) {
                    ...
                    if (element._id !== 'var') {
                        for (const subelement of this.objectsTemplates[element._id]) {
                            ...
                            if (subelement._id === 'structure') {
                                await this.setState(
                                    `nvl.${key}.${element._id}.${subelement._id}`,
                                    JSON.stringify(gvlStructure),
                                    true,
                                );
                            }
                        }
            ...
            for (const key in structure) {
                ...
                switch (structure[key].type) {
                    case 'BOOL':
                        ...
                        await this.setStateChangedAsync(`${baseId}.${key}.value`, {
                            val: Boolean(values[key]),
                            ack: true,
                        });
                        break;
                    ...
                    default:
                        this.log.error(
                            `updateDatabaseAsync: variable type <${structure[key].type}> not implemented yet`,
                        );
                }
            }
        } catch (err) {
     ...
     *
     * @param {string} locStr string to format
